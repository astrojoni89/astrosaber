...
class saberTraining(object):
    ...
    def train(self):
        popt_lam = self.train_lambda_set(objective_function=self.objective_function_lambda_set, training_data=self.training_data, test_data=self.test_data, noise=self.noise, lam1_initial=self.lam1_initial, p1=self.p1, lam2_initial=self.lam2_initial, p2=self.p2, lam1_bounds=self.lam1_bounds, lam2_bounds=self.lam2_bounds, iterations=self.iterations, MAD=self.MAD, eps=self.eps, learning_rate=self.learning_rate, mom=self.mom, window_size=5, iterations_for_convergence_training=10, ncpus=self.ncpus)


    def objective_function_lambda_set(self, function=self.single_cost, lam1, p1, lam2, p2, ncpus=None): #, training_data=None, test_data=None, header=None, check_signal_sigma=None, noise=None, velo_range=None, niters=None, iterations_for_convergence=None, add_residual=None, thresh=None, mask=None, get_all=True, dof=4, 
    ...
    def train_lambda_set(objective_function=self.objective_function_lambda_set, training_data=None, test_data=None, noise=None, lam1_initial=None, p1=None, lam2_initial=None, p2=None, lam1_bounds=None, lam2_bounds=None, iterations=100, MAD=None, eps=None, learning_rate=None, mom=None, window_size=5, iterations_for_convergence_training=10, mask=None, ncpus=None):
        ...
        for i in range(self.iterations):
            ...
            #lam1
            obj_lam1r, rchi2_lam1r, _ = objective_function(function=self.single_cost, self.lam1_r, self.p1, self.lam2_c, self.p2, ncpus=self.ncpus) # self.training_data, self.test_data, self.header, self.check_signal_sigma, self.noise, self.velo_range, self.niters, self.iterations_for_convergence, self.add_residual, self.thresh, self.mask, get_all=True, dof=4,
            obj_lam1l, rchi2_lam1l, _ = objective_function(function=self.single_cost, self.lam1_l, self.p1, self.lam2_c, self.p2, ncpus=self.ncpus)
            gd.D_lam1_trace[i] = (obj_lam1r - obj_lam1l) / 2. / self.eps

            #lam2
            obj_lam2r, rchi2_lam2r, _ = objective_function(function=self.single_cost, self.lam1_c, self.p1, self.lam2_r, self.p2, ncpus=self.ncpus)
            obj_lam2l, rchi2_lam2l, _ = objective_function(function=self.single_cost, self.lam1_c, self.p1, self.lam2_l, self.p2, ncpus=self.ncpus)
